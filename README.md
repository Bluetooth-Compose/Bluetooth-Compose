...
*Because just like Schrödinger's cat, you don't know what's alive in the AR space until you have a look! And because what's good for the goose is good for the gander, then let the goose be your watch-dog - if the dog barks, you know the cat's there, that's what Bluetooth-Compose is all about!*

### A Sweet Introduction to augmentation

Before we introduce Bluetooth-Compose, let's get a little excited by the convergence of Augmented Reality (AR) with Bluetooth Devices (BD) in general!
...
So as an introduction, you may have already seen IKEA's wonderful Augmented Reality app where you can choose a piece of furniture in it's catalogue and just point your phone at an empty space in your room where you would like to visualise it ... say a chair or a sofa. 
...
- If you want to value-add the space around you, let the device empower you in the context of where it is at, instead of you having to journey away from the device to a browser, just to find a means of engaging with it (more on 'Location Synchronicity' later!).
...
- Existing Bluetooth Devices can be retrofitted with Bluetooth-Compose capability by importing into an app it's custom [device name].bcodpkg Bluetooth-Compose-On-Demand Package, that is especially created for it.
- New Bluetooth Devices can be built that would deliver the same artefacts directly from the device itself to the app, using Bluetooth Extended Advertising.
...
And we do recognise, of course, that Bluetooth is just one mechanism of how devices can be Electronically Conspicuous, so we will be mindful of writing the Bluetooth-Compose specification so that it can exist as part of a complete federation of similar electronically-conspicuous yaml composition mechanisms.
...
- .bcodpkg, a custom zip-file file extension designating a 'Bluetooth-Compose-On-Demand Package' for transporting bluetooth-compose.yml files and associated artefacts. Double-clicking such a file would be one way of importing it into an app that utilises Bluetooth-Compose-On-Demand. (Other ways include retrieving the same artefacts directly from a Bluetooth Device, using that device's extended advertising capability, or using that device's extended advertising capability to identify a location on the web from where to download the aforementioned .bcodpkg file) 
- .NET Maui
- AR App, ARA
- AR Module, ARM
...
Yet despite being a private project - with a patent application pending - it was realised that inevitably, for the Electronically Conspicuous service to be consumed en-mass, the bluetooth-compose specification would have to develop as open-source, with the advantage being that whereas it could still be consumed by the original private context (and in doing so, with the LRBs themselves maintaining their own patent-pending commercial significances), that bluetooth-compose could also be consumed in a myriad of circumstances that are totally unconnected with the LRB technology ~ it would be a case of using Bluetooth-Compose to connect to the LRB technology if you wanted to use it, but otherwise, using Bluetooth-Compose in every other context that you wanted to without regard for the existence of the LRB nodes.
...
- Another example would be pulling out an Augmented Reality app, using object detection to identify a medical device on your bedside table, and by tapping on the detected object, automatically retrieving it's bluetooth-compose.yml file from the cloud, engaging with the device's augmented instrumentation and in doing so, associating it's data-feed with your particular health-service-provider in the cloud, so that you can then take a reading from the device, and relay that information to the health-service-provider's cloud against your own Personal Medical Event Hub (PMEH). 
...
- Furthermore, Bluetooth-Compose must be capable of being used in a non-AR context, such as on pages of apps without any AR capability, as well as in devices that don't have any UI at all. For example, a bluetooth-connected soil-moisture monitor, as a ble peripheral, must be capable of automatically generating it's services, characteristics, and descriptors, using Bluetooth-Compose, and whether or not it makes additional opportunities available to interested ble central devices by broadcasting an additional bluetooth-compose.yml using extended advertising, is immaterial.
- Yet any bluetooth device that uses extended advertising to broadcast it's own bluetooth-compose.yml file can be said 'to have AR capability' (indeed VR and XR capability as well), and that is because by doing so it delegates the ble peripheral device's AR, VR, and XR capabilities to the receiving ble central device. 
- And perhaps we need to introduce a new term called RR (Real Reality) where devices make use of bluetooth-compose information, but using traditional non-AR-VR-XR means of displaying it, such as rendering that information on a plain old app page. 

Of course, a full-blown discussion must include much detail about this adjacent but not unconnected topic of ```Augmented Reality```, where not only can devices be pictorially identified in the ```visual AR-space``` - with or without visual information that a user can read in a ```'Bluetooth-Head-Up Display' (BHUD)``` context similar to fighter-jet Head-Up Display (HUD) - but also incorporating the capacity of Bluetooth Device Object Detection that allows a user to visually scan a device, which once recognised using the Machine Learning mechanism of Object Detection - and typically through a user gesture that grants user-permission - would cause the app to autoconfigure to take a reading. 

*In this context the ble central device provides the instrumentation capability of the ble peripheral device!*
...
1. KOTLIN MULTIPLATFORM CONSUMPTION OF A SEPARATE UNITY AR MODULE<br />The creation of a Unity AR 'hello-world' module in it's own repository that is consumed by a Multiplatform hello-world world application that is separate in it's own repository but which loads that Unity AR module, which from here are collectively called the Bluetooth-Compose AR App ('BCAR app'), and which is capable of deploying to iOS and Android devices, with the initial intention being to ultimately deploy to the Google Play Store, and Apple App Store. 
     ...
     4. Acceptance Criteria: The functionality in the iOS and Android deployment of the stand alone Unity App will appear via git submodule deployment to the KMP Repo and upon build and deployment will be visible in both iOS and Android deployments of the KMP app. (We are going to hold-off making the AR Unity Module available as a separate binary, based upon IKEA's experience.)
...
6. THE CREATION AND CONSUMPTION OF A PRELIMINARY BLUETOOTH-COMPOSE.YML FILE WITH OBJECT DETECTION CAPABILITY<br />The creation of a bluetooth-compose.yml specification that the BCAR App can load and consume under manual user-selection, which bluetooth-compose.yml file will identify a web-based ML file that the AR module seeks, consumes, then uses, to identify that particular 3D Printed Bluetooth Device Container.
     ...
     2.  Acceptance Criteria: The bluetooth-compose.yml syntax must be specified, a toolkit must be created that allows a referenced ML file to be created and must include end-to-end instructions from creating the initial photographs of a bluetooth device in question, through training a model, to how a bluetooth-compose.yml file would allow the ML output to be consumed from the web so that it can be the basis of object detection in the BCAR App. For simplicity, the method of adding the bluetooth-compose.yml file to the BCAR Module, will be via the mechanism of that file being contained in a zipfile with the file-extension .bcodpkg ('Bluetooth-Compose-On-Demand Package), which when double-clicked shall be imported into the BCAR App, unpacked, and the bluetooth-compose.yml file being extracted and passed from the main app to the BCAR Module, via that module's interface.
