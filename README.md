...
- Yet any bluetooth device that uses extended advertising to broadcast it's own bluetooth-compose.yml file can be said 'to have AR capability', and that is because by doing so it delegates the ble peripheral device's AR capability to the receiving ble central device.
