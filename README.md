...
Furthermore, Bluetooth-Compose must be capable of being used in a non-AR context, such as on pages of apps without any AR, as well as in devices that don't have any UI at all. For example, a bluetooth-connected soil-moisture monitor, as a ble peripheral, must be capable of automatically generating it's services, characteristics, and descriptors, using Bluetooth-Compose, and whether or not it makes additional oppoprtunities available to inteested ble central devices by broadcasting an additional bluetooth-compose.yml using extended advertising, is immaterial.
