...
- Existing Bluetooth Devices can be retrofitted with Bluetooth-Compose capability by importing into an app it's associated [device name].bcodpkg Bluetooth-Compose-On-Demand Package, that is especially created for it.
- New Bluetooth Devices can be built that would deliver the same artefacts directly from the device itself to thje app, using Bluetooth Extended Advertising.
